...
public class GPSR_Planarization_RNGWithUnavailable implements AlgorFunc {
	...
	public GPSR_Planarization_RNGWithUnavailable(Algorithm algorithm){
...
/*
protected void paintNeighbors(){
	NetTopoApp app = NetTopoApp.getApp();
	WirelessSensorNetwork wsn = app.getNetwork();
	Painter painter = app.getPainter();
	
    // source node extended sensor node, thus, source node is also got
	Collection sensorNodes = wsn.getNodes("org.deri.nettopo.node.gpsr.SensorNode_GPSR",true); 
	...
	for(int i = 0; i < nodes.length; i++){
		int id_i = nodes[i].getID();
		List neighbor_i = nodes[i].getNeighbors();
		for (int j=0; j<neighbor_i.size();j++){
			int id_j = ((Integer)neighbor_i.get(j)).intValue();
